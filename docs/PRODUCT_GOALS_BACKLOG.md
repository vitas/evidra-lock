...
### 2. Policy & Validation
- Human-readable policy profile (`policy/profiles/ops-v0.1`): split into decision aggregator + focused rule files.
  - Polished hints stored in `data.rule_hints`.
  - Rule IDs follow stable `POL-*` / `WARN-*` convention.
...
- Single primary command: `evidra validate <path>` with auto-detection (terraform vs k8s). Keep `ops validate` as legacy alias if necessary.
...
- `evidra-mcp` requires explicit `--policy`/`--data`; resolve via flags > env > error.
