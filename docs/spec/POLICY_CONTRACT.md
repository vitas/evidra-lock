...
`policy/profiles/ops-v0.1` (shim + `policy/` directory + `data.json`) is the single authoritative profile. The runtime and CLI default to the files in this directory when no overrides are supplied via `--policy`/`--data` or `EVIDRA_POLICY_PATH`/`EVIDRA_DATA_PATH`. No other directories should be treated as authoritative.
