...
kubectl_apply_shape_unsupported(payload) if {
	defaults.has_key(payload, "initContainers")
	not is_array(object.get(payload, "initContainers", null))
}

kubectl_apply_shape_unsupported(payload) if {
	defaults.has_key(payload, "spec")
	not is_object(object.get(payload, "spec", null))
}

kubectl_apply_shape_unsupported(payload) if {
	defaults.has_key(payload, "metadata")
	not is_object(object.get(payload, "metadata", null))
}
